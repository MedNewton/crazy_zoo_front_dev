...
const SwapPage = () => {
  ...
  const [OtherTokenAddress , setOtherTokenAddress] = useState(chain && Othertokens[chain?.id][toCoin.symbol])
  ...
  useEffect(()=>{
    if( chain != undefined){
      if(fromCoin.symbol == 'zoo' ){
        ...
        console.log("USDC",Othertokens[chain?.id][toCoin.symbol])
        setIsZoo(true)
        setOtherTokenAddress(Othertokens[chain?.id][toCoin.symbol])
      } else {
        console.log("Buying Zoo")
        console.log("USDC",Othertokens[chain?.id][fromCoin.symbol])
        setIsZoo(false)
        setOtherTokenAddress(Othertokens[chain?.id][fromCoin.symbol])
      }
    }
  },[fromCoin, toCoin])
